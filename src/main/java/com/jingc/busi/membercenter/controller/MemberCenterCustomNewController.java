 ...
 */
@Controller
@RequestMapping("/memberCenterCustomNew/")
public class MemberCenterCustomNewController {
	@Resource
	...
	public ModelAndView custom(HttpSession session,
							   Integer type,//type:1.我定制的跟单 2.我被定制的记录 3.我指定的跟单 ... 5.跟单记录
							   Integer licenseId,
							   Integer pageNo,
							   Integer pageSize){
		...
		if(type == 1){
			...
			// 取到彩票种类类别
			List<StoreLicense> types = this.memberCenterService.getStoreLicenseList(mb.getStoreId());
			mv.addObject("ltype", types);
			mv.addObject("pagination", pagination);
			if(licenseId != null){
				mv.addObject("licenseId", licenseId);
			}
			mv.setViewName("membercenternew/myCustoming");
			return mv;
		}else if(type == 2){
			...
			mv.setViewName("membercenternew/myCustomed");
			return mv;
		}else if(type == 3){
...
//			temp.setMemberId(mb.getId());
			...
			mv.setViewName("membercenternew/myAppointed");
			return mv;
		}else if(type == 4){
			MergerAppoint temp = new MergerAppoint();
			...
			mv.addObject("ltype", types);
			mv.addObject("pagination",pagination);
			mv.setViewName("membercenternew/myAppointToMe");
			return mv;
		}
		/*
		 * 跟单记录 zh OK
		 */
			...
			pagination = this.memberCenterService.selectMergerCustomLog(mb, pageNo, pageSize);
			List<StoreLicense> types = this.memberCenterService.getStoreLicenseList(storeId);
			mv.addObject("ltype", types);
			mv.addObject("pagination", pagination);
			mv.setViewName("membercenternew/getCustomLog");
			return mv;
		}else{
			return null;
		}
