		...
		for ( var i = 0; i < schedules.length; i++ ) {
			...
			if ( date == obj.id.substring(0,8) ) {
				html += '<div class="sai_c '+zhcss+'" '+visibleHtml+' id="'+obj.id+'" host="'+obj.hostname+'" guest="'+obj.guestname+'">' +
		          ...
		          '<div class="duiz_wn fl tal">' +
						'<span class="fl tal">'+obj.guestname+'</span>' +
		                '<span class="fl tal lvs">vs</span>' +
		                '<span class="fl tal">'+obj.hostname+'</span>' +
		                '<span class="fl tal lvs '+gbc+' lanq_r">（'+obj.ggqf+'）</span>'+
		           '</div>' ;
				if ( spfDg ) {
					html += '<div class="duiz_ntz fl bcl danshi_d">' +
		             '<div class="dan_ico">' +
			          '<i class="triAngle"></i>' +
			          '<i class="text">单</i>' +
			          '</div>' ;
				} else {
					html += '<div class="duiz_ntz fl bcl">';
				}
			    html +=   		'<span class="duiz_ntz_l fl bgc" bet="0" betName="客胜">' +
			                	'<div class="s_qiudui fl tal">客胜</div>' +
			                    '<div class="s_qiudui fl tal">'+sfodds[1]+'</div>' +
			                '</span>' +
			           		'<span class="duiz_ntz_l fl bgc" bet="3" betName="主胜">' +
			                	'<div class="s_qiudui fl tal">主胜</div>' +
			                    '<div class="s_qiudui fl tal">'+sfodds[0]+'</div>' +
			                '</span>' +
			         '</div>' +
