...
$(document).ready(function () {
	...
	// 过关区域 2串1^8串1
	var plen = 0;
	if ( parseInt($("#playId").val()) == 6 ) {
		plen = guess.getMixedMaxPassNum(betCode);
	} else {
		plen = guess.getMaxPassNum();
	}
	plen = betCodes.length > plen ? plen : plen;
	...
	for ( var i = 0; i < betCodes.length; i++ ) {
		...
		html += '<tr>'+
            '<td width="65" height="33px" class="w_bright">'+dateUtils.getWeekDay2(dd)+cInfo[0].substring(8)+'</td>'+
            '<td width="83">'+sch.hostName+' </td>'+
            '<td width="84" class="w_bright">'+sch.guestName+'</td>'+
            '<td width="187" class="d_blue scroll_w">';
		var bets = cInfo[1].split(",");
		for ( var j = 0; j < bets.length; j++ ) {
			html += '<span class="zxgl_digt zxgll_red" bet="'+bets[j]+'">'+getBetName(parseInt($("#licenseId").val()), parseInt($("#playId").val()), bets[j])+'</span>';
		}
        html +=  '</td></tr>';
	}
	...
	$(".condition_w,.shezhi_wbtn").live('click', function(){
		...
		for ( var i = 0; i < betCodes.length; i++ ) {
			...
			var bets = cInfo[1].split(",");
			for ( var j = 0; j < bets.length; j++ ) {
				html += '<span class="zxgl_digt " bet="'+bets[j]+'">'+getBetName(parseInt($("#licenseId").val()), parseInt($("#playId").val()), bets[j])+'</span>';
			}
		    html +=     '<td width="47" class="zx_bd"><a href="javascript:void 0" class="d_red" name="coldRmvLnId">全</a></td>'+
		        '</tr>';
		}
		...
		for ( var i = 0; i < len; i++ ) {
			$("<option value='"+(i+1)+"'>"+(i+1)+"</option>").appendTo($("#coldStartVId"));
			$("<option value='"+(i+1)+"'>"+(i+1)+"</option>").appendTo($("#coldEndVId"));
		}
		// 已选条件
		$("#confUlId").empty();
		if ( null != pointer ) {
	...
	$("a[name=coldRmvLnId]").live('click',function(){
		...
		$(this).parent().parent().find(".zxgl_digt").addClass("zxgll_red");
		if ( $(this).parent().parent().find(".zxgll_red").length == len ) {
			$(this).parent().parent().find(".zxgl_digt").removeClass("zxgll_red");
		}
		...
		$("#coldSchId").find("tr").each(function(ii){
			...
			if ( $(this).find(".zxgll_red").length > 0 ) {
				$(this).find(".zxgll_red").each(function(index){
					html += getBetName(parseInt($("#licenseId").val()), parseInt($("#playId").val()), $(this).attr("bet"));
					conf += $(this).attr("bet");
				});
			} else {
				html += "#";
	...
	for ( var i = 0; i < list.length; i++ ) {
		...
		for ( var o = 0; o < betCodes.length; o++ ) {
			if ( codes.length > index && betCodes[o].split(":")[0] == codes[index].split(":")[0] ) {
				code += getBetName(parseInt($("#licenseId").val()), parseInt($("#playId").val()), codes[index].split(":")[1]) + "_";
				index++;
			} else {
				code += "*_";
			}
		...
		html += '<td width="95">'+code+'</td>';
		html += '<td width="95">'+new Number(list[i].baseBonus*list[i].multiple).toFixed(2)+'</td>';
