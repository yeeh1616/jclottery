...
function init() {
	// 生成号码
	var beforeHtml = '<h1><span>胆码</span>至少选择1个号码，至多选择1个号码</h1>'+
						'<ul class="">'+
					'<div class="dan-next">'+
	...
	beforeHtml = '<h1><span>拖码</span>胆码+拖码总个数不少于3个</h1>'+
					'<ul class="">'+
				'<div class="dan-next">'+
				'<div class="dan-wyl">遗漏</div></div>';
	for ( var i = 1; i < 15; i++ ) {
	...
	// 添加选中事件
	 $("#beforeDanAreaId").find("a[name=betN]").click(function(){
         if ( $(this).hasClass("active") ) {
                 ...
                 if ( $("#beforeDanAreaId").find("a[name=betN].active").length == 2 ) {
                         $(this).removeClass("active");
...
function calculte() {
	...
	if ( (beforeDanArray.length > 0 ) && (beforeDanArray.length + beforeTuoArray.length > 3 && beforeTuoArray.length >= 1) ) {
		//var betNum = dlt.calcDanTuoBetNum(beforeDanArray, beforeTuoArray);
		var combAlg = new CombAlgorithm();
		var betNum = combAlg.calcCombNum(beforeTuoArray.length,2-beforeDanArray.length);
