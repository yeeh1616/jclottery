...
function init() {
	// 生成号码
	var beforeHtml = '<h1><span>号码</span>至少选择1个</h1>'+
						'<ul class="">'+
					'<div class="dan-next">'+
					'<div class="dan-wyl">遗漏</div></div>';
	var D = ["1单","2单","3单","4单","5单","6单","7单","全单","全双"];
	for ( var i = 0; i < D.length; i++ ) {
		beforeHtml += '<li><a class="" name="betN" href="javascript:void 0" bet="'+D[i]+'">'+D[i]+'</a>'+
...
function calculte() {
	...
	if ( beforeArray.length >= 1 ) {
		var betNum = combAlg.calcCombNum(beforeArray.length,1);
