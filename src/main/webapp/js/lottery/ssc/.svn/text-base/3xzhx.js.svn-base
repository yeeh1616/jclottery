...
$(document).ready(function () {
	...
	$("#addToBetListBtnId").click(function(){
		if ( parseInt($("#betNumId").html()) > 0 ) {
			...
			var dansFs = betNum == "1" ? "三星直选单式" : "三星直选复式";
	...
	$("#radomSelDsBtnId").click(function(){
		...
		for ( var i = 0; i < v; i++ ) {
			var betCode = engine.random(1,10)+"|"+engine.random(1,10)+"|"+engine.random(1,10);
			var betNum = 1;
			var dansFs = "三星直选单式";
			$('<li betNum='+betNum+' betCode='+betCode+'><span class="qyspa">'+
					'<input name="" type="checkbox" value="" class="qycodlclia" />'+
...
function calc() {
	...
	if ( betCode != "" ) {
		...
		if ( betCode.split("|").length == 3 ) {
			betNum = engine.calcBetNum(4, betCode);
		}
	}
