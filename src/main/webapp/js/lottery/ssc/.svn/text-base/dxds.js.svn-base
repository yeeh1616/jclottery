...
$(document).ready(function () {
	...
	for ( var i = 0; i < betAreaV.length; i++ ) {
		...
		for ( var j = 0; j < 4; j++ ) {
			html += '<li>'+
	            '<p class="qyconQsya"><span class="qyconQsy2a">'+betS[j]+'</span></p>'+
	            '<p class="qyconQsy3">'+j+'</p>'+
	        '</li>';
		}
		html += '</ul>'+
		'</div>';
	...
	$("#addToBetListBtnId").click(function(){
		if ( parseInt($("#betNumId").html()) > 0 ) {
			...
			var dansFs = betNum == "1" ? "大小单双单式" : "大小单双复式";
	...
	$("#radomSelDsBtnId").click(function(){
		...
		for ( var i = 0; i < v; i++ ) {
			var betCode = betS[parseInt(engine.random(1,4))]+"|"+betS[parseInt(engine.random(1,4))];
			var betNum = 1;
			var dansFs = "大小单双单式";
			$('<li betNum='+betNum+' betCode='+betCode+'><span class="qyspa">'+
					'<input name="" type="checkbox" value="" class="qycodlclia" />'+
...
function calc() {
	...
	if ( betCode != "" ) {
		...
		if ( betCode.split("|").length == 2 ) {
			betNum = engine.calcBetNum(13, betCode);
		}
	}
