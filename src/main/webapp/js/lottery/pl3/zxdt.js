...
$(document).ready(function () {
	...
	$(calcA[0]).find("a[name=betN]").each(function(index){
		$(this).click(function(){
			if ( $(this).hasClass("active") ) {
				...
				if ( $(calcA[0]).find(".active").length == 2 ) {
					layer.alert("最多选择2个胆码！",0);
					return false;
				} else {
	...
	//加载遗漏数据
	loadOmitDate($("#licenseId").val()=='1'?'001003':'012003');
});
...
function calculte() {
	...
	var betNum = 0;
	if ( betDanArray.length + betTuoArray.length > 3 && betDanArray.length > 0 ) {
		betNum = pl3.calcZxDt(betDanArray, betTuoArray);
	}
