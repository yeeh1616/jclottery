	...
	$("#line1AreaId").find(".qecondlConrp1").each(function(index){
		$(this).click(function(){
			if ( $(this).children().eq(0).hasClass("qecondlConrp2b") ) {
				...
				if ( $("#line1AreaId").find(".qecondlConrp2b").length > 0 ) {
					popupConfirm("温馨提示","只允许选择一个胆码！");
					return false;
				}
...
function calc() {
	...
	if ( $("#line1AreaId").find(".qecondlConrp2b").length > 0 && $("#line2AreaId").find(".qecondlConrp2b").length > 1 ) {
		var betCode = "(";
		$("#line1AreaId").find(".qecondlConrp2b").each(function(index){
			betCode += $(this).attr("bet") + ",";
		});
		...
		betNum = engine.calcBetNum(21, betCode.substring(0, betCode.length - 1));
	}
