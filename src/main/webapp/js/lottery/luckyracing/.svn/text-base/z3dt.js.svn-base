...
function calc() {
	...
	var gjCodes = $("#gjPanelId").find(".qyconQsy2b");
	var yjCodes = $("#yjPanelId").find(".qyconQsy2b");
	if ( gjCodes.length > 0 && yjCodes.length > 1 && gjCodes.length + yjCodes.length > 3 ) {
		for ( var i = 0 ; i < gjCodes.length; i++ ) {
			betCode += $(gjCodes[i]).html() + ",";
		}
		betCode = betCode.substring(0, betCode.length - 1) + ")";
		...
		betNum = engine.calcBetNum(41, betCode.substring(0, betCode.length - 1));
	}
