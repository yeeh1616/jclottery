$(document).ready(function () {
	$("input[name=buyChasName]").eq(2).parent().hide();
	updateKjInfo();
	...
	// 开奖描述信息
	try {
		var csi = parseInt($("#currentIssueId").html().substring(6,8));
		$("#kjDescId").html("今天已开"+(csi-1)+"期，还剩"+(85-csi)+"期");
	} catch (ex){}
...
function updateKjInfo() {
	...
	$.ajax({
	    ...
	    success: function (data){
			var objs = eval("("+data+")");
			$("#historyIssueId").html(objs[0].issue.substring(2));
			...
			for ( var i = 0; i < objs.length; i++ ) {
				$('<ul>'+
	                	'<li class="qycodrba2 zicol">'+objs[i].issue.substring(2)+'</li>'+
	                	'<li class="qycodrba3 zicola">'+objs[i].code.split("-")[0]+'</li>'+
	                	'<li class="qycodrba3 zicola">'+objs[i].code.split("-")[1]+'</li>'+
	                	'<li class="qycodrba3 zicola">'+objs[i].code.split("-")[2]+'</li>'+
...
function clock() {
	...
	if ( sec > 0 ) {
		...
		if ( min > 0 ) {
			...
			if ( parseInt(currentIssue.substring(6,8),10) == 84 ) {
				$("#stopMId").html("--");
				$("#stopSId").html("--");
				$("#stopMId2").html("--");
				$("#stopSId2").html("--");
	...
	if ( option == "1" ) {
		...
		$("#chasingIssListId").find("ul").each(function(index){
			if ( $(this).find("input[type=checkbox]:checked").length > 0 ) {
				issueNum++;
				totalMoney += parseInt($(this).find("span[name=currPay]").html());
			}
