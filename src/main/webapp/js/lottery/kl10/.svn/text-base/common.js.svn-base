...
$(document).ready(function () {
			    ...
			    success: function (data){
					if ( data == '-1' ) {
						...
						if ( pay > balance ) {
							...
							$("#sureZjTdId").hide();
							var playName = getPlayName($("#chsPlayerId").val());
	...
	bonus =getPlayBonus($("#chsPlayerId").val());
});

/**
 * 玩法中文名称
 */
function getPlayName(player) {
	var name = "";
	switch( player ) {
	case "x1st":
		name = "选一数投";
...
function callback(objs) {
	...
	$(".qiu").eq(5).html(objs[0].code.split("-")[5]);
	$(".qiu").eq(6).html(objs[0].code.split("-")[6]);
	$(".qiu").eq(7).html(objs[0].code.split("-")[7]);
	
	$("#kjListAreaId").empty();
	for ( var i = 0; i < objs.length; i++ ) {
		$('<ul>'+
            	'<li class="qycodrba2 zicol">'+objs[i].issue+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[0]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[1]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[2]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[3]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[4]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[5]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[6]+'</li>'+
            	'<li class=" zicola" style="width:7%">'+objs[i].code.split("-")[7]+'</li>'+
