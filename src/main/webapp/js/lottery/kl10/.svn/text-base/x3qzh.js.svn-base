...
$(document).ready(function () {
	...
	$(".qyconQsy2").eq(0).find(".qyconQsy2a").click(function(){
		if ( $(this).hasClass("qyconQsy2aaa") ) {
			...
			$(".qyconQsy2").eq(2).find(".qyconQsy2a").each(function(){
				if ( $(this).html() == val ) {
					$(this).removeClass("qyconQsy2aaa");
				}
			});
	...
	$(".qyconQsy2").eq(2).find(".qyconQsy2a").click(function(){
		if ( $(this).hasClass("qyconQsy2aaa") ) {
			$(this).removeClass("qyconQsy2aaa");
		} else {
			$(this).addClass("qyconQsy2aaa");
	...
	$("#addToBetListBtnId").click(function(){
		if ( parseInt($("#betNumId").html(),10) > 0 ) {
			...
			var zjCodes = $("ul[name=betPanel]").eq(2).find(".qyconQsy2aaa");
			...
			betCode = betCode.substring(0, betCode.length - 1) + "|";
			for ( var i = 0 ; i < zjCodes.length; i++ ) {
				betCode += $(zjCodes[i]).html() + ",";
			}
			betCode = betCode.substring(0, betCode.length - 1);
	...
	$("#radomSelDsBtnId").click(function(){
		...
		for ( var i = 0; i < v; i++ ) {			
			var betCodes = random(3,21);
			var betCode = betCodes[0] + "|" + betCodes[1] + "|" + betCodes[2];
			var betNum = 1;
			var dansFs = "选三前直";
			$('<li betNum='+betNum+' betCode='+betCode+'><span class="qyspa">'+
					'<input name="" type="checkbox" value="" class="qycodlclia" />'+
...
function calc() {
	...
	var yjCodes = $("ul[name=betPanel]").eq(1).find(".qyconQsy2aaa");
	var zjCodes = $("ul[name=betPanel]").eq(2).find(".qyconQsy2aaa");
	if ( (gjCodes.length > 0) && (yjCodes.length>0) && (zjCodes.length>0) ) {
		betNum = gjCodes.length * yjCodes.length * zjCodes.length;
	}
