		    ...
		    success: function (data){
				if ( data == '-1' ) {
					...
					if ( (pay > balance) && oo == "1") {
						...
						if ( oo == "1" ) {
							if ( $("#pageNameId").val() == "zjqdssc" ) { // 单式上传
								dsscFormSubmit();
							} else {
								formSubmit();
							}
...
function ajaxFileUpload() {
	$("#fileuplodid").uploadify({
		...
		'displayData': 'speed',
		'method' : 'POST',
		'scriptData' : {"licenseId":$('#licenseId').val(),"playId":"5","passNum":$("#dsPassTId").find("input[type=radio]:checked").val(),"schNum":$("#dsSchDivId").find("input[type=checkbox]:checked").length},
		'auto' : true,
		'fileDesc' : '*.txt',//上传文件类型说明
		'fileExt' : '*.txt',
		'sizeLimit' : 1024 * 1024 * 1024 * 10, //上传文件大小限制为5m
		onSelect: function (event, queueID, fileObj){//动态加载上传组件的参数
			$("#fileuplodid").uploadifySettings('scriptData',{"licenseId":$("#licenseId").val(),"playId":"5","passNum":$("#dsPassTId").find("input[type=radio]:checked").val(),"schNum":$("#dsSchDivId").find("input[type=checkbox]:checked").length});
		},
