...
function load() {
	...
	guess = new SingleSchedule(parseInt($("#playId").val()));
	...
	// 过关区域 2串1^8串1
	var plen = betCodes.length > 15 ? 15 : betCodes.length;
	...
	$("#bonusCalId").html('<p>预计奖金：'+minBonus+' ^ '+highestBonus+'</p>' + 
			'<p>赔率和：'+new Number(minAddSp).toFixed(2)+' ^ '+new Number(maxAddSp).toFixed(2)+'</p>' + 
			'<p>赔率积：'+new Number(minMulSp).toFixed(2)+' ^ '+new Number(maxMulSp).toFixed(2)+'</p>');
	var html = "";
	$("#schAreaId").empty();
	for ( var i = 0; i < betCodes.length; i++ ) {
		var cInfo = betCodes[i].split(":");
