 ...
 * Added by Luochang at 2015/7/10 Version 1.0
 */
...
function Filter() {
	...
	this.shrink = function(list, expressions){
		for ( var i = 0; i < expressions.length; i++ ) {
			...
			case this.ROUTINE_SCHNUM_0:
				...
				break;
			case 120:
				list = this.filtSchNum(list, expressions[i].expr, "0");
				break;
			case 121:
				list = this.filtSchNum(list, expressions[i].expr, "1");
				break;
			case 122:
				list = this.filtSchNum(list, expressions[i].expr, "2");
				break;
			case 123:
				list = this.filtSchNum(list, expressions[i].expr, "3");
				break;
	...
	this.filtIntgSum = function(list, expression) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			var sum = 0;
			//var codes = list[i].join(",").replaceAll(",#","").split(",");
			var _codes = list[i].join(",").split(",");
			var codes = new Array();
	...
	this.filtBreakPoint = function(list, expression) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			//var codes = list[i].join(",").replaceAll("#","").replaceAll(",{1,}", ",").split(",");
	...
	this.filtEvenNum = function(list, expression) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			var index = 0;
			//var codes = list[i].join(",").replaceAll("#","").replaceAll(",{2,}", ",").split(",");
			var _codes = list[i].join(",").split(",");
			var codes = new Array();
			for ( var j = 0; j < _codes.length; j++ ) {
				if ( _codes[j] != '#' ) {
	...
	this.filtCtnNum = function(list, expression, bet) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			if ( parseFloat(exp[0]) <= comp && parseFloat(exp[2]) >= comp ) {
				retList.push(list[i]);
			}
		}
		return retList;
	...
	this.filtCtnNumXX = function(list, expression, bet0, bet1) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			var comp = 1;
			for ( var j = 0; j < codes.length-1; j++ ) {
				if ( (codes[j] == bet0 && codes[j+1] == bet1) || (codes[j] == bet1 && codes[j+1] == bet0) ) {
					index++;
				} else {
					if ( index > comp ) {
						comp = index;
	...
	this.filtNeighSum = function(list, expression) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			for ( var j = 0; j < codes.length - 1; j++ ) {
				index += Math.abs(parseInt(codes[i],10) - parseInt(codes[i+1],10));
			}
			if ( parseInt(exp[0],10) <= index && parseInt(exp[2],10) >= index ) {
				retList.push(list[i]);
			}
	...
	this.filtNeighMul = function(list, expression) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			for ( var j = 0; j < codes.length - 1; j++ ) {
				index += parseInt(codes[i],10) * parseInt(codes[i+1],10);
			}
			if ( parseInt(exp[0],10) <= index && parseInt(exp[2],10) >= index ) {
				retList.push(list[i]);
			}
	...
	this.oddFilte = function(list, expression, odex) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			for ( var j = 0; j < codes.length; j++ ) {
				if ( codes[j] != "#" ) {
					var oddArray = oddMap.get(j).split(",");
					var odd = getBetOdds(j, codes[j]);
					oddArray.sort(this.compare);//降序
					if ( oddArray[odex - 1] == odd ) {
						counter++;
					}
				}
			}
	...
	this.coldSubFilte = function(list, exp) {
		...
		for ( var i = 0; i < list.length; i++ ) {
			...
			var codes = list[i].join(",").split(",")
			for ( var j = 0; j < codes.length; j++ ) {
				if ( (codes[j] != "#") && (map.get((j+1)+"").indexOf(codes[j]) > -1) ) {
					len++;
				}
			}
	...
	this.compareASC = function (_val1,_val2) {
		var base1 = 1;
		var codes1 = _val1.join(",").split(",");
		for ( var j = 0; j < codes1.length; j++ ) {
			if ( codes1[j] != "#" ) {
				var odd = getBetOdds(j, codes1[j]);
				base1 *= parseFloat(odd);
			}
		}
		var base2 = 1;
		var codes2 = _val2.join(",").split(",");
		for ( var j = 0; j < codes2.length; j++ ) {
			if ( codes2[j] != "#" ) {
				var odd = getBetOdds(j, codes2[j]);
				base2 *= parseFloat(odd);
			}
		}
		if ( base1 > base1 ) {
			return 1;
