  ...
  //提款提交
		...
		$.ajax({
		   ...
		   success: function (moneyData){
			  if((moneyData * 1) < (currMoney * 1)){
			  	...
			  	//表单提交
					$("#pageForm").remove();
					$("<form id='pageForm' action='/memberCenter/doGetMoneyZfb.jhtml' method='post'>" 
	...
	**********************************************************************/
	...
	$("#putmoneySubmit").click(function(){
		...
		//表单提交
		$("#pageForm").remove();
		$("<form id='pageForm' action='/memberCenter/putMoneyZfbSure.jhtml' method='post'>" 
				+ "<input type='hidden' value='" + currMoney + "' name='currMoney' />"
	...
	$("#putmoneySureSubmit").click(function(){
		...
		//表单提交
		$("#pageForm").remove();
		$("<form id='pageForm' action='/memberCenter/doPutMoneyZFBSure.jhtml' method='post' target='_blank'>" 
				+ "<input type='hidden' value='" + currMoney + "' name='currMoney' />"
