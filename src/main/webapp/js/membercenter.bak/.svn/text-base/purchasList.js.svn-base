$(document).ready(function () {
  ...
  $("[name='deleteOrder']").click(function(){
  	...
  	popupConfirm("提示信息", "是否确认删除"+$this.attr('orderId')+"号方案?", true,function(){
	  	...
	  	$.ajax({
		    type: "post",
		    url: "/memberCenter/deleteOrder.do",
		    ...
		    success: function (data){
			    if ( data == "1" ){
			    	...
			    	$this.parent().html('<a href="/memberCenter/orderDetail.jhtml?orderId='+$(this).attr('orderId')+'&LicenseId='+$(this).attr('LicenseId')+'">详情</a>');
			    } else {
