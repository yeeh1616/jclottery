...
<div class="w_gcjl">
  ...
  <c:forEach var="list" items="${pagination.list}" varStatus="varStatus">
    ...
    <td>${list.customizingMemberName}</td>
    <td>${list.customModeName}</td>
	 	<c:choose>
    ...
    <div class="pagetotal">
		<div class="ptotal">                                
			<form id="reportFormz" name="reportForm" action="/memberCenterCustomNew/custom.jhtml?type=2" method="post"></form>
    ...
    和分数，并不改变跟单排名，也可以自由退出跟单。如跟单用户余额不足时，则视为自愿放弃"自动跟单"（指定跟单人除外）再次定制跟单时。须重新定制;使用"定制跟单"功能后,会员可以正常使
    用其他合买功能</div>
  </div>    
    <!--具体信息end-->
  </div>
...
<script type="text/javascript" src="/js/membercenternew/mergerCustom.js"></script></body>
</html>
