...
<div class="common_w">
<div class="hall-top">
  <div class="sitepath fl"> 您当前的位置：<a href="/memberCenterNew/index.jhtml">用户中心</a> > <a href="/memberCenterNew/capital.jhtml">资金管理</a> > 账户明细 </div>
</div>
<!--用户中心-->
...
<div class="w_gcjl_l">
	<ul>
		<li><a href="/memberCenterNew/accountDetail.jhtml" class="active">账户明细</a></li>
	</ul>
	<span>显示最近一周记录</span>
	<div class="clear"></div>
</div>
<div class="w_gcjl_choose">
	<form id="reportForm" name="reportForm" action="/memberCenterNew/accountDetail.jhtml" method="post">
       	   我要筛选
    ...
    <div class="w_gcjl_bg">
	  ...
	  <tr>
	    ...
	    <td align="center" bgcolor="#FFFFFF"<c:choose><c:when test="${list.typeName eq '出账'}"> class="hblue"</c:when><c:otherwise> class="hred"</c:otherwise></c:choose>>${list.typeName}</td>
	    <td align="center" bgcolor="#FFFFFF">${list.categoryName2}</td>
	    <td align="center" bgcolor="#FFFFFF">${list.stateName}</td>
	    <td align="center" bgcolor="#FFFFFF"<c:choose><c:when test="${list.typeName eq '出账'}"> class="hgreen"</c:when><c:otherwise>class="hred"</c:otherwise></c:choose>>
	    <c:if test="${list.type == 1}">-</c:if><fmt:formatNumber value="${list.money}" pattern="#,###.##"  minFractionDigits="2"/>
	    </td>
	    <td align="center" bgcolor="#FFFFFF" class="hred"><fmt:formatNumber value="${list.balance}" pattern="#,###.##"  minFractionDigits="2"/></td>
