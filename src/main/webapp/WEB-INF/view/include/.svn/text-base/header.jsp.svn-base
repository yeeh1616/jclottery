      ...
      <div>${domain.netUrl }</div>
          ...
          <ul>
          	<c:set var="url" value="${pageContext.request.requestURI}" scope="page"></c:set>
		        <li><a href="/">首页</a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/lottery/') > -1 }">cur</c:if> zkx" href="javascript:void 0">购彩<i></i></a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/merger/') > -1 || fn:indexOf(url, '/memberCenterCustomNew/') > -1}">cur</c:if> zkx" href="javascript:void 0">合买<i></i></a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/copyorder/') > -1 }">cur</c:if>" href="/copyorder/index.jhtml">复制</a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/pass/') > -1 }">cur</c:if>" href="/pass/index.jhtml?succFlag=1">过关</a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/tubiao.cp020.com/') > -1 || fn:indexOf(url, '/lotteryopen/') > -1}">cur</c:if> zkx" href="javascript:void 0">数据<i></i></a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/tuijian.cp020.com/') > -1 || fn:indexOf(url, '/recommend/') > -1}">cur</c:if> zkx" href="javascript:void 0">推荐<i></i></a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/datacenter/') > -1 }">cur</c:if>" href="/datacenter/jsbf.jhtml">比分</a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/storeintro/') > -1 }">cur</c:if>" href="/storeintro/index.jhtml">店铺</a></li>
		        <li><a class="<c:if test="${fn:indexOf(url, '/wap/') > -1 }">cur</c:if>" href="/wap/index.jhtml?${domain.netUrl}">手机</a></li>
		      </ul>
        </div>
    ...
    <!-- 彩种选择begin -->
     <div class="box" id="navbox" style="height:0px;opacity:0;overflow:hidden;">
      <div class="cont xzcz" style="display:none;" id="commLicenAreaId">
       ...
       <div class="cont h_xial sec_sj" style="display:none;"> <a href="http://tubiao.cp020.com/?${domain.netUrl}" target="_blank">图表走势</a> <a href="/lotteryopen/index.jhtml">彩票开奖</a> </div>
       <div class="cont h_xial sec_tj" style="display:none;"> <a href="/recommend/index.jhtml">店内推荐</a> <a href="http://tuijian.cp020.com/?${domain.netUrl}"  target="_blank">专家</a></div>
    </div>
    <!-- 彩种选择end -->
